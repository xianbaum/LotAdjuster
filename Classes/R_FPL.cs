...
namespace LotExpander
{
    public class R_FPL
    {
        ...
        public R_FPL(IPackageFile LotPackage, IPackedFileDescriptor Descriptor)
        {
            PFD = Descriptor;
            ...
            if (uBlockID != 0xAB4BA572)
                if (LETools.ErrorChecking)
                    throw new InvalidDataException("Invalid FPL: Block ID");
            ...
            if ((bBlockNameLen & 0x80) != 0)
                if (LETools.ErrorChecking)
                    throw new InvalidDataException("Invalid FPL: Block Name Length");
            string sBlockName = BR.ReadString();
            if (sBlockName != "cFencePostLayer")
                if (LETools.ErrorChecking)
                    throw new InvalidDataException("Invalid FPL: Block Name");
        ...
        public void Change(int XAddLow, int XAddHigh, int YAddLow, int YAddHigh, int iWidthNew, int iHeightNew)
        {
            ...
            for (int i = 0; i < iItemCount; i++)
            {
                ...
                int iFloorNumber = BR.ReadInt32();
                BW.Write(iFloorNumber);
                iStructureSize += 4;
                ...
                if (YNew == YFix)
                {
                    ...
                    iStructureSize += 4;
                    if (Test_PrintDebugInfo)
                        Debug.Print("FPL Y = {0} -> {1}", YOld, YNew);
                }
                else
                {
                    // ToDo: Handle off-world coordinates by deleting?
