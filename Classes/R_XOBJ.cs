...
namespace LotExpander
{
    public class R_XOBJ
    {
        ...
        public R_XOBJ(IPackageFile LotPackage, IPackedFileDescriptor Descriptor, int iMinLevel, int iMaxLevel)
        {
            PFD = Descriptor;
        ...
        private void SetXY(float XNew, float YNew)
        {
            ...
            if (Test_PrintDebugInfo)
                if ((XOld == XNew) && (YOld == YNew))
                    Debug.Print("    XOBJ X = {0} Y = {1}", XOld, YOld);
                else
                    Debug.Print("    XOBJ X = {0} -> {1} Y = {2} -> {3}", XOld, XNew, YOld, YNew);
        }
